...
class Admin_model extends CI_Model {
    ...
    public function checkLogin() {
        ...
        $query      = $this->db->get_where('employees', array('email_id' => $strEmailAddress, 'password' => $strPassword), 1);
        $objResult  = (true == valArr($query->result())) ? current($query->result()) : NULL;
        
        if (true == valObj($objResult, 'stdClass') && true == valStr($objResult->id)) {
            $this->session->set_userdata(array('id' => $objResult->id, 'email_id' => $objResult->email_id,'city_id'=>$objResult->city_id,'branch_id'=>$objResult->branch_id, 'user' => $objResult));
            return true;
        } else {

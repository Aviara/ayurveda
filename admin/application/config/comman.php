...
function merge_intersect_array($arr1, $arr2) {
    ...
    if (false == is_array($arr1) || false == is_array($arr2))
        return array();
...
function rekeyArray($strKeyFieldName, $arrarrUnkeyedData) {
    if (false == valArr($arrarrUnkeyedData))
        return $arrarrUnkeyedData;
...
function checkSessionTimeout($strPortalName, $intInactiveTime, $strRedirectUrl) {
    if (0 === $intInactiveTime)
        $intInactiveTime = determineDefaultSessionTimeOut($strPortalName);
...
function setArrayDataToObject($arrstrData, $object) {
    if (false == valArr($arrstrData) || false == is_object($object))
        return false;
...
function valArr($arr, $intCount = 1, $boolCheckForEquality = false) {
    ...
    if (true == $boolCheckForEquality && true == $boolIsValid)
        $boolIsValid = ( $intCount == count($arr) ) ? true : false;
    return $boolIsValid;
}
...
function usortCallbackAsc($arr1, $arr2) {
    if ($arr1['cmp_value'] == $arr2['cmp_value'])
        return 0;
...
function arrayValuesExistsInArray($arrmixSearchValues, $arrmixMasterArray, $boolMatchAll = false) {
    ...
    if (false == valArr($arrmixSearchValues) || false == valArr($arrmixMasterArray))
        return false;
...
function generateDateRangeArray($strStartDate, $strEndDate) {
    ...
    if (true === is_string($strStartDate))
        $strStartDate = strtotime($strStartDate);
    if (true === is_string($strEndDate))
        $strEndDate = strtotime($strEndDate);

    if ($strStartDate > $strEndDate)
        return generateDateRangeArray($strEndDate, $strStartDate);
...
function convertNumberToWords($fltNumber, $strUnit = 'Dollars') {
    ...
    if (NULL !== $intFraction && is_numeric($intFraction) && 0 < $intFraction) {
        ...
        if (true == array_key_exists($strUnit, $arrstrDecimalUnit))
            $strWords .= ' ' . $arrstrDecimalUnit[$strUnit];
    }
...
function logOpeningRequest($intPsProductId, $strModuleName = NULL, $strActionName = NULL, $objManagementCompany = NULL, $objWebsite = NULL, $objCompanyUser = NULL) {
    ...
    if ('production' != CConfig::get('environment'))
        return;
    if ('production' == CConfig::get('environment') && ( 5 != rand(2, 300) ))
        return;

    if (false == defined('CONFIG_SQL_LOGGING_JOB_QUEUE_SERVER') || false == defined('CONFIG_SQL_LOGGING_JOB_QUEUE_URI'))
        return;
...
function logClosingRequest($intPsProductId, $strModuleName = NULL, $strActionName = NULL, $objManagementCompany = NULL, $objWebsite = NULL, $objCompanyUser = NULL) {
    ...
    if (true == isset($GLOBALS['query_times']) && true == isset($GLOBALS['end_times']) && true == isset($GLOBALS['queries']) && true == isset($GLOBALS['query_databases'])) {
...
function sortTwoDimensionalArray($arrMixData, $strIndex, $strOrder, $boolNatSort = false, $boolCaseSensitive = false) {
    ...
    if (false == valArr($arrMixData) || 0 == count($arrMixData))
        return $arrMixData;
